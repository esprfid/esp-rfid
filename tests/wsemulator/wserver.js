...
function remove(uidKey) {
            for (var i=0; i < users.length; i++) {
               if (users[i].uid === uidKey) {
				     console.log("[ INFO ] Removed: " + users[i]);
                     users = users.slice(i);
        }
      }
    }
...
function sendUserList(page) {
    ...
    for (var i = (page - 1) * 15; i < page * 15; i++) {
		if (typeof users[i] !== "undefined") {
			datatosend.list[i] = users[i];
		}
	}
	wss.broadcast(datatosend);
	wss.broadcast(var res = {"command":"result","resultof":"userlist","result": true});
}
  ...
  ws.on('message', function incoming(message) {
	...
	switch (obj.command) {
		...
		case "configfile" :
			...
			console.log("[ INFO ] New configuration recieved" + configfile);
			break;
		case "userlist" :
			...
			break;
		default :
			console.log("[ WARN ] Unknown command " + obj)
  });
});